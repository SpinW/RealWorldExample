...
% Yb2Ti2O7 with the magnetic Hamiltonian proposed in the following paper: 
% http://journals.aps.org/prx/abstract/10.1103/PhysRevX.1.021002

%% Create crystal structure
% To create the cubic crystal structure of Yb2Ti2O7,
...
% We setup the number of triangular faces to be produced by the plot command
% using swpref. Each atom is an icosahedron, where each face is subdivided 
% into triangles nmesh-times (see swplot.icomesh function). Thus nmnesh=3
% will make all sphere to have 1280 faces. npatch determines the number of 
% subdivision of the circle that is used to generate cylinders an arrows 
% (swplot.arrow and swplot.cylinder).
pref = swpref;
pref.set({'nmesh', 'npatch'}, {3, 50})

% Take a note of the options in `swpref`.

%% Plot cubic environment of Yb3+
...
% property of the spinw object). The unitcell() function can return selected
...
% Yb2Ti2O7. The arrow pointing from an atom to another denotes the 
...
%% Or we can customise the plot even more
...
plot(ybti,'tooltip',false,'atomMode','mag','atomColor','gold','bondColor','red')

% Choose 4 atoms which make up a tetrahedra. Make a note of their x, y, z
% positions (in fractional lattice co-ordinates). Hint, you might want to
% turn on the tooltip
